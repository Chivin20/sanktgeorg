...
class PDFController extends Controller
{
    ...
    {
        //ceate a new dompdf instance
        $dompdf = new Dompdf();

        //load html content
        
//set options and render the pdf
        $dompdf->loadHtml($html);
        $dompdf->setPaper('A4', 'portrait');
        $dompdf->render();

        //generate a random filenamr
        $calendar = 'pdf_' . uniqid() . '.pdf';

        //save the pdf to the public directory
        $pdfPath = public_path('pdfs/' . $calendar);
        $dompdf->output($pdfPath);
