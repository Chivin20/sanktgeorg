

<?php $__env->startSection('content'); ?>

<div class="row">
    <div class="container">
        <div class="col-12" style="font-family: 'Quicksand', sans-serif;">
            <h1 class="text-danger text-center mt-3 pt-3" style="font-size: 28px; text-align:center;"><b>Third Term (2022/2023) Calendar for Nursery and Primary Section</b></h1>
        </div>
    </div>
</div>



<!-- About Principal of School -->
<div class="row">
    <div class="container">
        <div class="col-12 container-sm" style="margin-top: 20px; margin-bottom: 20px; font-family: 'Quicksand', sans-serif;" >
            <div class="row">
           
                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h5 style = "font-style:oblique; font-weight:bold;">Dates</h5>
                        </div>
                        <div class="col-9">
                            <h5 style = "font-style:oblique; font-weight:bold;">Activities</h5>
                        </div>
                    </div>
                </div>
                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Thursday 27th April, 2023</h6>
                        </div>
                        <div class="col-9">
                            
                            <p>Staff Resumption / Submission of lesson notes</p>
                    </div>
                </div>
                </div>
                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Monday 1st May, 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Workers Day (Holiday)</p>
                        </div>
                    </div>
                </div>


                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Tuesday 2nd May, 2023</h6>
                        </div>
                        <div class="col-9">
                          
                            <p>Pupils Resumption </p>
                        </div>
                    </div>
                </div>
                	
	


                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Monday 8th May 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Submission of CAT 1 and CAT 2</p>


                        </div>
                    </div>
                </div>
                
                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Monday 15th May 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>CAT 1</p>


                        </div>
                    </div>
                </div>

                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Monday 22nd May 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Science Activity (making of wind vane and making of liquid soap)</p>


                        </div>
                    </div>
                </div>

                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Wednesday 24th May 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Home economics Practical (Bead making/Fruit salad)</p>


                        </div>
                    </div>
                </div>

                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Monday 29th May 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Holiday for Children’s Day (May 27th – Saturday)</p>


                        </div>
                    </div>
                </div>
               
                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Monday 5th – Thursday 8th June 2023	</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>CAT 2</p>

                        </div>
                    </div>
                </div>

                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Friday 9th – Monday 19th June, 2023	</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Open Day / Mid Term Break</p>


                        </div>
                    </div>
                </div>

                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Monday 3rd July 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Submission of Exam Questions</p>


                        </div>
                    </div>
                </div>

                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Monday 10th July 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Inter class Maths and English Competition</p>


                        </div>
                    </div>
                </div>

                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Tuesday 11th – Friday 14th July 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Revision / End of session Party Preparation</p>


                        </div>
                    </div>
                </div>

                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Monday 17th – Friday 21st July 2023	</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Examination / End of Term Preparation</p>


                        </div>
                    </div>
                </div>

                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Wesnesday 26th July 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Vacation of Pupils with  Graduation ceremony</p>


                        </div>
                    </div>
                </div>

                <div class="shadow p-3 mb-2 bg-body rounded">
                    <div class="row">
                        <div class="col-3">
                            <h6>Friday 28th July 2023</h6>
                        </div>
                        <div class="col-9">
                           
                            <p>Teachers Vacation</p>


                        </div>
                    </div>
                </div>

            </div>
        </div>

       <div class="col-sm-12 col-sm-3, d-flex justify-content-end" style = "padding-bottom: 25px;"> 
        <form action = "<?php echo e(route('generate-pdf')); ?>" method="GET">
        <button class="btn btn-primary rounded-pill"> Download </button>
        </form>

        <?php if(isset($calendar)): ?>
        <a href ="<?php echo e(pdfs('pdfs/' . $calendar)); ?>" download> Download PDF </a>
        <?php endif; ?>
    </div>
    </div>
</div>
<!-- end of about Principal of School-->
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\user\Desktop\sanktgeorg\resources\views/priviewcalendarthird.blade.php ENDPATH**/ ?>